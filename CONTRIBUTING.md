...
Thank you for your interest in FLUXION. Bug reports, documentation fixes, a missing operation on
hyperduals and whole new parts of the derivation layer are all welcome.
...
ask on [our Discord](https://discord.gg/MEJg3zzt7). No question is too small, and a five minute
...
merged pull requests. **You are welcome.** A student who wants to see how a derivative is carried by
the number itself is exactly who we wrote this for, and nobody is going to hold a deadline against
you.
...
So say so. In the issue, or on [Discord](https://discord.gg/MEJg3zzt7): "I have two weeks and I
...
issue](https://github.com/codereckons/fluxion/labels/good%20first%20issue) label lists work that is
...
```bash
cmake --preset gcc
cmake --build build/gcc --target unit.hyperdual.exe
ctest --preset gcc
```

One target per test file, and CMake globs the test directories, so a new `.cpp` needs no CMake edit.
`test/failure/` holds the sources that must **not** compile, which is how a refusal is tested.
FLUXION pulls EVE, which is heavy: build the one target you are working on rather than the suite.
...
- **the documentation of what you touched**, with the `fluxion-doxygen` target still silent, since a
...
**A wrong derivative.** The fix and its test in the same commit.
...
order, and the input values. A Godbolt link that reproduces it replaces all three.

**Work on an open issue.** The [open issues](https://github.com/codereckons/fluxion/issues) are all
fair game, and help on any of them is welcome. Say on the issue that you are taking it, so that two
people do not write the same patch.

**Anything else.** Ideas outside this list are welcome too. Bring one up on
[Discord](https://discord.gg/MEJg3zzt7) before you write it: a design that has been talked through
...
cmake --build build/gcc --target fluxion-doxygen
```
