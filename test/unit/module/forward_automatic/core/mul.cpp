...
auto der_mul(auto a, auto b)
{
  using flx::der;
  using flx::val;
  return val(a) * der(b) + der(a) * val(b);
}

//==================================================================================================
// Tests for flx::mul
//==================================================================================================
TTS_CASE_WITH("Check behavior of flx::mul(eve::wide) for real values",
...
<typename T>(T const& w0, T const& w1)
{
  ...
  //  valder + valder
  TTS_EQUAL(wv0 * wv1, var(w0 * w1 , der_mul(wv0,wv1)) );
  TTS_EQUAL(wv0 *  sv, var(w0 * s  , der_mul(wv0, sv)) );
  TTS_EQUAL(sv  * wv1, var(s  * w1 , der_mul(sv ,wv1)) );
  TTS_EQUAL(sv  *  sv, var(s  * s  , der_mul(sv,sv)  ) );
