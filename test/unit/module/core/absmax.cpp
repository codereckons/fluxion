...
<typename T>(tts::type<T>)
{
  ...
  TTS_EXPR_IS( flx::diff(eve::absmax)(T())  , T  );
  TTS_EXPR_IS( flx::diff(eve::absmax)(v_t()), v_t);
};
...
<typename T>(T const& a0)
{
  ...
  auto dabsmax=[&](auto e) { return !!; }
  TTS_ULP_EQUAL( flx::diff(eve::absmax)(a0), map(dabsmax, a0), 0.5);
};
