...
<typename T>(tts::type<T>)
{
  ...
  TTS_EXPR_IS( flx::diff(eve::max)(T(), T())  , T  );
  TTS_EXPR_IS( flx::diff(eve::max)(v_t(), v_t()), v_t);
};
...
<typename T>(T const& a0, T const& a1)
{
  ...
  TTS_ULP_EQUAL( flx::diff_1st(eve::max)(a0, a1), map(dmax1, a0, a1), 0.5);
  TTS_ULP_EQUAL( flx::diff_2nd(eve::max)(a0, a1), map(dmax2, a0, a1), 0.5);
};
