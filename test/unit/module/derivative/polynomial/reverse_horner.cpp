...
<typename T>(tts::type<T>)
{
  ...
  TTS_EXPR_IS( flx::derivative(eve::reverse_horner)(T(), T(), T())  , T  );
  TTS_EXPR_IS( flx::derivative(eve::reverse_horner)(v_t(), v_t(), v_t()), v_t);
};
...
<typename T>(T const& x, T const& a0, T const& a1, T const& a2, T const& a3)
{
 ...
 auto dreverse_horner = [&](auto e, auto , auto c, auto b, auto a) { return c+2*b*e+3*a*e*e; };
  TTS_ULP_EQUAL(flx::derivative_1st(eve::reverse_horner)(x, a3, a2, a1, a0), map(dreverse_horner, x, a3, a2, a1, a0), 0.5);
};
