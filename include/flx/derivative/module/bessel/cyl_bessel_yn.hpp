...
namespace eve::detail
{
                                            ...
                                            , T x) noexcept
  {
    if constexpr(N == 2)
    {
      ...
      {
        return average(cyl_bessel_yn(dec(nn), x), -cyl_bessel_yn(inc(nn), x));
      }
      else return apply_over(flx::derivative_2nd(cyl_bessel_yn), nn, x);
    }
    else
    {
      EVE_ASSERT( N == 2, "cyl_bessel_yn derivative is only implemented relative to second parameter");
