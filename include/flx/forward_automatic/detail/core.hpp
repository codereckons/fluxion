...
namespace flx::detail
{
                                              ...
                                              ) noexcept
  {
    ...
    if      constexpr(!is_valder_v<Z1> &&  is_valder_v<Z2>) return r_t(z, v_t(der(z2)));
    else if constexpr( is_valder_v<Z1> && !is_valder_v<Z2>) return r_t(z, v_t(der(z1)));
                                              ...
                                              ) noexcept
  {
    ...
    if      constexpr(!is_valder_v<Z1> &&  is_valder_v<Z2>) return r_t(z, v_t(z1) * v_t(der(z2)));
    else if constexpr( is_valder_v<Z1> && !is_valder_v<Z2>) return r_t(z, v_t(der(z1)) * v_t(z2) );
    else
    {
                                              ...
                                              ) noexcept
  {
    ...
    if constexpr( is_valder_v<Z1> && !is_valder_v<Z2>) return r_t(z, v_t(der(z1)/val(z2)));
    else
    {
      auto invvz2 = eve::rec(eve::sqr(val(z2)));

      if constexpr(!is_valder_v<Z1> &&  is_valder_v<Z2>) return r_t(z, -v_t(z1)*v_t(der(z2))*invvz2);
      else
      {
        auto v1 = v_t(val(z1)); auto d1 = v_t(der(z1));
        auto v2 = v_t(val(z2)); auto d2 = v_t(der(z2));
        auto dr = (v2 * d1 - v1 * d2)*invvz2;
        return r_t(z, dr);
      }
    }
  }
