...
namespace flx
{
  ...
  struct hypot_t : eve::callable<hypot_t, Options>
  {
    ...
    flx_CALLABLE_OBJECT(hypot_t, hypot_);
  };

//======================================================================================================================
//! @hypottogroup functions
//! @{
//!   @var hypot
...
//!   namespace flx
//!   {
//!      template<concepts::hyperdual_like T0, oncepts::hyperdual_like ... Ts > constexprT hypot(T0 z0,  Ts... zs) noexcept;
//!   }
//!   @endcode
//!
//!   **Parameters**
...
namespace flx::_
{
  ...
  FLX_FORCEINLINE constexpr auto hypot_(flx_DELAY(), O const& o, Z0 z0, Zs... zs) noexcept
  {
    auto val = eve::hypot[o](e0(z0), e0(zs)...);
    if constexpr(concepts::base<flx::as_hyperdual_like_t<Z0, Zs...>>)
    {
      return val;
    }
    else
    {
      auto r = flx::sqrt(flx::sqr(z0)+(flx::sqr(zs)+...));
      e0(r) = val;
      return r;
    }
  }
