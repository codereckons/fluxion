...
namespace flx
{
  ...
  struct sinc_t : eve::elementwise_callable<sinc_t, Options>
  {
    ...
    flx_CALLABLE_OBJECT(sinc_t, sinc_);
};
...
//!   namespace flx
//!   {
//!      template<flx::concepts::hyperdual_like T> constexprT sinc(T z) noexcept;
//!   }
//!   @endcode
//!
//!   **Parameters**
...
namespace flx::_
{
  ...
  FLX_FORCEINLINE constexpr auto sinc_(flx_DELAY(), O const&, Z z) noexcept
  {
    auto val = eve::sinc(e0(z));
    ...
    else
    {
      ...
      auto comp_ders = [&](auto  x){
        ...
        {
          auto x2 = eve::sqr(x);
          ders[2] =  if_else(test, -eve::third(eve::as(val)), -eve::fma(2*x, c, (x2-2)*s)*rx2*rx);
          if constexpr(ord == 2) return;
          else
          {
            ders[3] = if_else(test, eve::zero,  eve::fms(3*(x2-2), s, x*(x2-6)*c)*eve::sqr(rx2));
            if constexpr(ord == 3) return;
            else
            {
              ders[4] =if_else(test, b_t(1.0/5),  eve::fma(4*x*(x2-6), c, (24+x2*(x2-12))*s)*eve::sqr(rx2)*rx);
              return;
            }
          }
        }
