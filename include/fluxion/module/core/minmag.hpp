...
namespace flx
{
  ...
  struct minmag_t : eve::callable<minmag_t, Options>
  {
    ...
    flx_CALLABLE_OBJECT(minmag_t, minmag_);
  };
...
namespace flx::_
{
  ...
  FLX_FORCEINLINE constexpr auto minmag_(flx_DELAY(), O const& o, T0 z0, T1 z1, Ts... zs) noexcept
  {
    using r_t = flx::as_hyperdual_like_t<T0, T1, Ts...>;
    if constexpr(sizeof...(Ts) == 0) // 2 parameters
    {
      r_t z(flx::if_else(flx::is_less[o](flx::abs(z0), flx::abs(z1)), z0, z1));
      return z;
    }
    else // N > 2 parameters
    {
